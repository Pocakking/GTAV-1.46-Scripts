...
void __EntryFunction__()
{
	...
	while (true)
	{
		if (func_6() || Global_1677610 == 1)
		{
			func_5();
		}
		if (func_1(&uLocal_21, &Global_287847))
		{
			func_5();
		}
		SYSTEM::WAIT(0);
			...
			while (iVar2 <= iVar6)
			{
				if (iVar2 < iVar5)
				{
					...
					if (iVar2 >= Var3.f_4 && iVar2 != 27)
					{
						...
						{
							if (NETWORK::_NETWORK_ACCESS_TUNABLE_FLOAT_HASH(iVar7, Var3.f_5, &fVar0))
							{
							...
							else if (NETWORK::_NETWORK_ACCESS_TUNABLE_INT_HASH(iVar7, Var3.f_5, &iVar1))
							{
...
int func_6()
{
	...
	if (func_7() != 0)
	{
		if (SCRIPT::_GET_NUM_OF_INSTANCES_OF_SCRIPT_WITH_NAME_HASH(func_7()) == 0)
		{
			return 1;
		}
	}
