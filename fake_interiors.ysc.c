...
void func_1()
{
	...
	if (iLocal_29 != -1 && func_20(iLocal_29))
	{
		...
		if (SYSTEM::VDIST2(vVar1, vVar0) < IntToFloat(func_18(iLocal_29)))
		{
			...
			if (bVar2)
			{
				UI::SET_RADAR_AS_INTERIOR_THIS_FRAME(GAMEPLAY::GET_HASH_KEY(func_13(iLocal_29)), vVar1.x, vVar1.y, func_12(iLocal_29), func_11(iLocal_29));
				func_10(iLocal_29);
			}
		}
	}
	if (NETWORK::NETWORK_IS_GAME_IN_PROGRESS())
	{
		if (func_2())
		{
			if (GAMEPLAY::IS_BIT_SET(iLocal_32, 1))
			{
				GAMEPLAY::CLEAR_BIT(&iLocal_32, 1);
			}
			if (UI::IS_PAUSE_MENU_ACTIVE())
			{
				if (!GAMEPLAY::IS_BIT_SET(iLocal_32, 0))
				{
					UI::SET_MINIMAP_COMPONENT(15, 1, -1);
					GAMEPLAY::SET_BIT(&iLocal_32, 0);
				}
			}
			else if (GAMEPLAY::IS_BIT_SET(iLocal_32, 0))
			{
				UI::SET_MINIMAP_COMPONENT(15, 0, -1);
				GAMEPLAY::CLEAR_BIT(&iLocal_32, 0);
			}
		}
		else if (!GAMEPLAY::IS_BIT_SET(iLocal_32, 1))
		{
			UI::SET_MINIMAP_COMPONENT(15, 0, -1);
			GAMEPLAY::CLEAR_BIT(&iLocal_32, 0);
			GAMEPLAY::SET_BIT(&iLocal_32, 1);
		}
	}
}
