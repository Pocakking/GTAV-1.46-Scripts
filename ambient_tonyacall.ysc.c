...
void __EntryFunction__()
{
	...
	if (SCRIPT::_GET_NUM_OF_INSTANCES_OF_SCRIPT_WITH_NAME_HASH(joaat("ambient_tonyacall")) > 1)
	{
	...
	switch (iLocal_40)
	{
		case 0:
			if (ENTITY::DOES_ENTITY_EXIST(iLocal_38) && !ENTITY::IS_ENTITY_DEAD(iLocal_38, 0))
			{
				if (ENTITY::IS_ENTITY_AT_COORD(iLocal_38, vLocal_49, 1f, 1f, 1f, 0, 1, 0))
				{
					...
					AI::TASK_PERFORM_SEQUENCE(iLocal_38, uVar2);
					AI::CLEAR_SEQUENCE_TASK(&uVar2);
					iLocal_40 = 1;
				}
			}
		...
		case 1:
			if (ENTITY::DOES_ENTITY_EXIST(iLocal_38) && !ENTITY::IS_ENTITY_DEAD(iLocal_38, 0))
			{
				...
				{
					iVar4 = (GAMEPLAY::GET_RANDOM_INT_IN_RANGE(0, 65535) % 2);
					if (iVar4 == 0)
					{
	...
	if (ENTITY::DOES_ENTITY_EXIST(iVar1) && !ENTITY::IS_ENTITY_DEAD(iVar1, 0))
	{
		...
		if (iVar2 == joaat("ig_tonya"))
		{
			...
			if (!func_23(vLocal_49))
			{
				...
				{
					AI::TASK_FOLLOW_NAV_MESH_TO_COORD(iLocal_38, vLocal_49, 1f, -1, 0.25f, 0, fLocal_50);
					PED::SET_PED_KEEP_TASK(iLocal_38, 1);
				}
				return 1;
			}
