				...
				switch (iLocal_2)
				{
						...
						if (ENTITY::DOES_ENTITY_HAVE_DRAWABLE(iScriptParam_6))
						{
							if (SCRIPT::_GET_NUM_OF_INSTANCES_OF_SCRIPT_WITH_NAME_HASH(joaat("michael2")) > 0)
							{
								if (Global_89758)
								{
									if (STREAMING::HAS_ANIM_DICT_LOADED("map_objects"))
									{
										ENTITY::PLAY_ENTITY_ANIM(iScriptParam_6, "P_Abat_roller_1_open", "map_objects", 1f, 0, 1, 0, 0, 0);
										iLocal_2 = 3;
									}
								}
							}
					...
					case 3:
						...
						if (ENTITY::DOES_ENTITY_EXIST(iLocal_3))
						{
							...
							{
								vVar2 = { vVar0 - vVar1 };
								ENTITY::SET_ENTITY_COORDS(iLocal_3, vVar1 + func_1(vVar2) * FtoV(GAMEPLAY::GET_FRAME_TIME()) * Vector(fVar3, fVar3, fVar3), 1, false, 0, 1);
							}
							else
							{
								Global_89759 = 1;
