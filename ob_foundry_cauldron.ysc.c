...
void __EntryFunction__()
{
	...
	while (true)
	{
		...
		{
			if (BRAIN::IS_OBJECT_WITHIN_BRAIN_ACTIVATION_RANGE(iScriptParam_5) && SCRIPT::_GET_NUM_OF_INSTANCES_OF_SCRIPT_WITH_NAME_HASH(joaat("finalec1")) == 0)
			{
				switch (iLocal_2)
				{
					case 0:
									...
									if (ENTITY::DOES_ENTITY_EXIST(iLocal_4))
									{
										...
										if (INTERIOR::IS_VALID_INTERIOR(iVar0))
										{
											if (INTERIOR::IS_INTERIOR_READY(iVar0))
											{
												if (INTERIOR::IS_INTERIOR_SCENE())
												{
													iLocal_3 = GRAPHICS::START_PARTICLE_FX_LOOPED_ON_ENTITY("scr_obfoundry_cauldron_steam", iLocal_4, 0f, 0f, 0f, 0f, 0f, 0f, 1065353216, 0, 0, 0);
												}
											}
										}
									}
